...
def show(username):
    user = User.get_or_none(User.name == username) # check whether user exist in database
    if user:
...
def follow(idol_id):
    ...
    if current_user.follow(idol):
        if current_user.follow_status(idol).is_approved:
            flash(f"You follow {idol.name}", "primary")
        else:
            flash(f"You send request to follow {idol.name}", "primary")
...
def unfollow(idol_id):
    ...
    if current_user.unfollow(idol):
        flash(f"You unfollow {idol.name}", "primary")
        return redirect(url_for('users.show', username=idol.name))
    else:
